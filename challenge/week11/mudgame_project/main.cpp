...
int main() {
  ...
  while (1) {  // 사용자에게 계속 입력받기 위해 무한 루프
    ...
    if (user_input == "상") {
      // 위로 한 칸 올라가기
      user_y -= 1;
      my_user.DecreaseHP(1);  // 움직일 때마다 HP 1씩 감소
    ...
    // 사용자가 하를 입력했을 때
      ...
      // checkXY를 이용해 유효한 좌표인지 확인하고, 맞으면 내려가고 아니면
      // 다시돌아가기
      bool inMap = checkXY(user_x, mapX, user_y, mapY);
      if (inMap == false) {
        cout << "맵을 벗어났습니다. 다시 돌아갑니다." << endl;
    ...
    // HP 체크하기 (0이하면 종료, 그렇지 않다면 계속 진행)
    if (!CheckUser(my_user)) {
      cout << "HP가 0 이하가 되었습니다. 게임을 종료합니다." << endl;
      break;
    }
...
void displayMap(int map[][mapX], int user_x, int user_y) {
  ...
  for (int i = 0; i < mapY; i++) {
    ...
    for (int j = 0; j < mapX; j++) {
      ...
      // 다르면 지도 출력 (0은 빈공간, 1은 아이템, 2는 적 , 3은 포션, 4는 물약,
      // 5는목적지)
      else {
...
void checkState(int map[][mapX], int user_x, int user_y, User& user) {
  switch (map[user_y][user_x]) {
    ...
    case 2:
      ...
      user.DecreaseHP(2);  // 감소하는 객체 동작 사용
      break;
    case 3:
